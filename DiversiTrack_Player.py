...
score = st.number_input("Enter your final score (after playing rounds):", min_value=0)
...
current_round = round_data.get("round", "Unknown")

if st.button("Submit Score"):
    if name and score:
        leaderboard_doc = leaderboard_ref.get()
        leaderboard_data = leaderboard_doc.to_dict() if leaderboard_doc.exists else {}
        scores = leaderboard_data.get("scores", [])
        scores.append({"name": name, "score": score})
        leaderboard_ref.set({"scores": scores})
        st.success("✅ Score submitted!")
    else:
        st.error("Please enter your name and score.")

st.info(f"🎯 Current Round (set by Admin): Round {current_round}")
